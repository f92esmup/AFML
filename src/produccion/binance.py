...
class BinanceConnector:
    ...
    def _setup_leverage(self) -> None:
        ...
        try:
            self._client.futures_change_leverage(
                ...
                leverage=int(self._config.apalancamiento)
            )
            log.info(f"Apalancamiento configurado a {self._config.apalancamiento}x para {self._config.simbolo}")
    ...
    def get_account_info(self) -> bool:
        ...
        try:
            ...
            log.debug(f"Información de cuenta actualizada - Balance: {self._balance}, Equity: {self._equity}")
            return True
