...
class Preprocesamiento:
    ...
    def __init__(self, config: Config) -> None:
        ...
        log.debug("Preprocesamiento inicializado con la configuración proporcionada.")


    def run(self, df: pd.DataFrame) -> tuple[pd.DataFrame, StandardScaler]:
        """ metodo para ejecutar el preprocesamiento """
        log.info("Iniciando pipeline de preprocesamiento.")
    ...
    def _interpolacion(self) -> pd.DataFrame:
        ...
        log.info(f"Interpolando datos usando el método: '{self.interpol_method}'.")
    ...
    def _continuidad(self) -> pd.DataFrame:
        ...
        """
        log.info("Verificando continuidad del índice de tiempo.")
        if not isinstance(self.df.index, pd.DatetimeIndex):
            log.error("El índice del DataFrame no es de tipo DatetimeIndex.")
        ...
        if not missing_timestamps.empty:
            log.info(f"Se encontraron {len(missing_timestamps)} timestamps faltantes. Rellenando huecos...")
            # Reindexar el DataFrame. Esto crea filas con NaN para los huecos.
            self.df = self.df.reindex(full_index)
        else:
            log.debug("El índice es continuo. No se encontraron huecos.")
    ...
    def _calculo_indicadores(self) -> pd.DataFrame:
        ...
        log.debug(f"Columnas tras cálculo de indicadores: {list(self.df.columns)}")
    ...
    def _eliminar_faltantes(self) -> pd.DataFrame:
        ...
        log.info(f"Eliminando filas con valores NaN. Se eliminaron {initial_rows - final_rows} filas.")
        return self.df
    
    def _scaler(self) -> StandardScaler:
        """ métdo para calcular el scaler """
        log.info("Ajustando el StandardScaler a los datos.")
        # Creamos el objeto StandardScaler:
        scaler = StandardScaler()
        # Ajustamos el scaler a los datos (aprendiendo los parámetros):
        scaler.fit(self.df)
        log.debug("StandardScaler ajustado con éxito.")
        return scaler
